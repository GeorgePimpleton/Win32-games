...
{
   g_pGame = new GameEngine( hInstance, TEXT( "Space Out 3" ), TEXT( "Space Out 3 a: Showing Off Your Game with Demo Mode" ),
...
{
   if ( pSpriteDying->GetBitmap( ) == g_pBMissileBitmap || pSpriteDying->GetBitmap( ) == g_pJMissileBitmap ||
        pSpriteDying->GetBitmap( ) == g_pTMissileBitmap )
   {
      if ( !g_bDemo )
      {
         PlaySound( ( PCTSTR ) IDW_SMEXPLODE, g_hInstance, SND_ASYNC | SND_RESOURCE | SND_NOSTOP );
      }
