...
BOOL GameInitialize(HINSTANCE hInstance)
{
   ...
   _pGame = new GameEngine(hInstance,
                           TEXT("Meteor Defense"),
                           TEXT("Example Game: Meteor Defense"),
                           IDI_ICON, 600, 450);
   if (_pGame == NULL)
   {
      return FALSE;
...
void GameMenu(WPARAM wParam)
{
   switch (LOWORD(wParam))
   {
      ...
      case IDM_HELP_ABOUT:
         DialogBox(_pGame->GetInstance(), MAKEINTRESOURCE(IDD_ABOUT),
                   _pGame->GetWindow(), (DLGPROC) DlgProc);
         return;
   }
}
...
void SpriteDying(Sprite * pSprite)
{
   ...
   if (pSprite->GetBitmap() == _pMeteorBitmap)
   {
      ...
      PlaySound((LPCSTR)IDW_EXPLODE, _hInstance,
                SND_ASYNC | SND_RESOURCE | SND_NOSTOP);
