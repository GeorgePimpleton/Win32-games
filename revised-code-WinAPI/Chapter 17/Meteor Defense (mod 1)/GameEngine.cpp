                   ...
                   PSTR szCmdLine, int iCmdShow)
{
  ...
  int          iTickCount;
   ...
   if (GameInitialize(hInstance) == TRUE)
   {
      // initialize the game engine
      if (GameEngine::GetEngine()->Initialize(iCmdShow) == FALSE)
      {
         return FALSE;
      ...
      while (TRUE)
      {
         ...
         {
            // process the message
            // if the message is WM_QUIT, end the message loop
...
GameEngine::GameEngine(HINSTANCE hInstance, PCWSTR szWindowClass, PCWSTR szTitle,
                       WORD wIcon, int iWidth, int iHeight)
{
...
BOOL GameEngine::Initialize(int iCmdShow)
{
   ...
   wc.hCursor         = LoadCursor(m_hInstance, MAKEINTRESOURCE(IDC_CURSOR));
   ...
   {
      MessageBoxW(NULL, L"Unable to initialize Main Window!", L"ERROR",
                 MB_ICONERROR | MB_OK);
      return FALSE;
   }

   // calculate the window size and position based upon the game screen size
   int iWindowWidth   = m_iWidth  + GetSystemMetrics(SM_CXFIXEDFRAME) * 2;
   int iWindowHeight  = m_iHeight + GetSystemMetrics(SM_CYFIXEDFRAME) * 2 +
                                    GetSystemMetrics(SM_CYCAPTION);

   // add the size for a main menu if there is one
   if (wc.lpszMenuName != NULL)
   {
      iWindowHeight += GetSystemMetrics(SM_CYMENU);
   }

   // set the  position in the center of the main display
    int iWindowPosX   = (GetSystemMetrics(SM_CXSCREEN) - iWindowWidth) / 2;
    int iWindowPosY   = (GetSystemMetrics(SM_CYSCREEN) - iWindowHeight) / 2;
   ...
   if (m_hWindow == NULL)
   {
      MessageBoxW(NULL, L"Unable to create Main Window!", L"ERROR",
                 MB_ICONERROR | MB_OK);
      return FALSE;
   }
