...
BOOL Bitmap::Create(UINT resID, HINSTANCE inst)
{
   ...
   m_bitmap = (HBITMAP) LoadImageW(GetModuleHandle(nullptr), MAKEINTRESOURCE(resID), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
...
BOOL Bitmap::Create(HWND wnd, LONG width, LONG height, COLORREF crColor)
{
   HDC dc = GetDC(wnd);
